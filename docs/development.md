...
They are wrappers and nothing more — there is no build step that only exists inside one, so
`dotnet build` on its own is always equivalent. CI runs the underlying commands directly, and
shellchecks these.
...
deploy/                install.sh, the unit file, the example settings and env files
scripts/               one-line wrappers: build, test, format, run
```
...
- **Line length.** Roslyn has no line-length rule and the formatter does not wrap, so
  `max_line_length` is checked by a step in CI that reads the same 110 from nowhere but its own
...
`.github/workflows/ci.yml` runs `build`, `test` and `format`, plus the line-length check,
`shellcheck deploy/install.sh`, `docker build`, and the test suite a second time under a
comma-decimal locale. A green local run is a green build.
